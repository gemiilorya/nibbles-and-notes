  ...
  return (
    <div className="min-h-screen">
      ...
      <div className='flex items-center gap-4 mt-5'>
        ...
        <div>
          <h4 className='font-open-sans font-extrabold text-[30px]'>{profile?.username || 'User'}</h4>
          {editingBio
            ? <input value={bio} onChange={(e) => setBio(e.target.value)} />
            : <p onClick={() => { setBio(profile?.bio || ''); setEditingBio(true) }}>
                {profile?.bio || 'Add a bio...'}
              </p>
          }
          {editingBio && <button onClick={handleSaveBio}>Save</button>}
        </div>
        <div className='ml-15'>
      ...
      {recipes.length === 0 && (
      <button className='bg-white text-black text-center font-open-sans py-5 px-12.5 rounded-full mt-10 block mx-auto w-fit' onClick={() => navigate('/add')}>Create Post</button>
      )}
      <div className="p-4">
        {recipes.length === 0
          ? <p className="text-center text-white mt-10 text-4xl font-open-sans">No posts yet.</p>
          : recipes.map(recipe => (
